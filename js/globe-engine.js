/* ═══════════════════════════════════════════════════════════
   NEXXORE TERMINAL — 3D GLOBE INTELLIGENCE ENGINE
   Drop-in replacement for the Leaflet MapEngine, rendered on a
   real 3D WebGL globe (globe.gl / three.js). Preserves the full
   public API (init, map, flyTo, getStats, toggleLayer, loadLayer,
   loadQuakes) and every key point from the flat map — conflicts,
   shipping, chokepoints, sanctions, energy, weather, military,
   corridors, LNG, grain, semis, rare-earths, pipelines, cables,
   disputed/FTA zones, nuclear, naval, quakes.

   If WebGL or globe.gl is unavailable, terminal.html falls back to
   the Leaflet engine automatically — so the map never breaks.
   ═══════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  // Popup HTML mirrors the Leaflet engine's tooltips (shown on hover).
  function pop(html) { return '<div class="nx-globe-pop">' + html + '</div>'; }

  var GlobeEngine = {
    globe: null,
    map: null,               // API-compat alias (terminal.html reads MapEngine.map)
    _el: null,
    layersLoaded: {},
    // Normalized records per layer: { kind:'point'|'path'|'arc'|'ring', ... }
    markers: {
    ...
    supported: function () { return typeof Globe !== 'undefined' && webglOK(); },
    ...
    init: function (containerId) {
      ...
      var g = Globe()
        ...
        .showGraticules(true)
        // Hex-dotted country landmasses — the "3D world graph" look.
        .hexPolygonResolution(3).hexPolygonMargin(0.32).hexPolygonAltitude(0.005)
        .hexPolygonColor(function () { return 'rgba(56,189,248,0.42)'; })
        // points
        .pointsData([]).pointLat('lat').pointLng('lng')
        .pointColor('color').pointAltitude('alt').pointRadius('radius')
        .pointLabel('label').pointsMerge(false).pointsTransitionDuration(0)
        // surface polylines (routes / pipelines / cables / corridors / grain)
        .pathsData([]).pathPoints('coords').pathPointLat(function (p) { return p[0]; })
        .pathPointLng(function (p) { return p[1]; })
        .pathColor('color').pathStroke('stroke').pathLabel('label')
        .pathPointAlt(0.006).pathTransitionDuration(0)
        // arcs (oil flows) — animated dashes for the "live feed" feel
        .arcsData([]).arcStartLat('sLat').arcStartLng('sLng')
        .arcEndLat('eLat').arcEndLng('eLng').arcColor('color')
        .arcStroke('stroke').arcLabel('label')
        .arcDashLength(0.4).arcDashGap(0.2).arcDashAnimateTime(1500)
        .arcAltitudeAutoScale(0.4)
        // rings (conflict pulses + disputed / FTA zones)
        .ringsData([]).ringLat('lat').ringLng('lng').ringColor('ringColor')
        .ringMaxRadius('maxR').ringPropagationSpeed('speed').ringRepeatPeriod('period');

      g(el);

      // Sizing to the container.
      var self = this;
      function resize() {
        if (!el) return;
        g.width(el.clientWidth || 800);
        g.height(el.clientHeight || 500);
      }
      resize();
      window.addEventListener('resize', resize);
      if (window.ResizeObserver) { try { new ResizeObserver(resize).observe(el); } catch (e) {} }

      // Gentle auto-rotate; stops on user interaction.
      ...
      if (controls) {
        ...
        ['mousedown', 'touchstart', 'wheel'].forEach(function (ev) {
          el.addEventListener(ev, function () { controls.autoRotate = false; }, { passive: true });
        });
      }
      g.pointOfView({ lat: 20, lng: 30, altitude: 2.0 }, 0);

      // Country outlines as hex dots (async — globe still works if it fails).
      fetch('https://unpkg.com/globe.gl/example/datasets/ne_110m_admin_0_countries.geojson')
        .then(function (r) { return r.json(); })
        .then(function (geo) { if (geo && geo.features) g.hexPolygonsData(geo.features); })
        .catch(function () {});

      this.globe = g;
      this.map = g; // compat alias

      // Default layers — same set the flat map loaded on boot.
      ['conflicts', 'chokepoints', 'shipping', 'sanctions', 'energy', 'weather'].forEach(function (n) {
        self.loadLayer(n);
      });
      this.loadQuakes();
      // Live feed — re-pull USGS quakes on an interval so the globe keeps updating.
      if (!window.matchMedia || !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        this._quakeTimer = setInterval(function () { self.loadQuakes(); }, 120000);
      }
      this._render();
      return g;
    },
    ...
    loadLayer: function (name) {
      ...
      var fn = '_add' + name.charAt(0).toUpperCase() + name.slice(1);
      if (typeof this[fn] === 'function') { this[fn](); this._render(); }
    },

    toggleLayer: function (name, btn) {
      this.layerVisible[name] = !this.layerVisible[name];
      if (btn) btn.classList.toggle('active', this.layerVisible[name]);
      if (!this.layersLoaded[name] && this.layerVisible[name]) { this.loadLayer(name); return; }
      this._render();
    },

    flyTo: function (region) {
      var regions = {
        GLOBAL: [20, 10, 2.4], AMERICAS: [15, -80, 1.9], EUROPE: [50, 10, 1.5],
        ASIA: [30, 100, 1.9], INDIA: [22, 78, 1.4], MENA: [25, 45, 1.5]
      };
      var r = regions[region];
      if (r && this.globe) this.globe.pointOfView({ lat: r[0], lng: r[1], altitude: r[2] }, 900);
    },

    getStats: function () {
      var m = this.markers, cnt = function (a) { return a.length; };
      return {
        conflicts: m.conflicts.filter(function (x) { return x.kind === 'point'; }).length,
        shipping: cnt(m.shipping),
        quakes: cnt(m.quakes),
        military: cnt(m.military),
        corridors: cnt(m.tradeCorridors),
        pipelines: cnt(m.pipelines),
        cables: cnt(m.submarineCables),
        naval: cnt(m.navalFleets),
        lngTerminals: cnt(m.lngTerminals),
        semiconductors: cnt(m.semiconductors),
        rareEarths: cnt(m.rareEarths),
        nuclear: cnt(m.nuclearFacilities),
        disputed: cnt(m.disputedZones),
        fta: cnt(m.ftaZones),
        grain: cnt(m.grainRoutes)
      };
    },

    // ── aggregate visible layers into the four globe primitives ──
    _render: function () {
      if (!this.globe) return;
      var pts = [], paths = [], arcs = [], rings = [];
      var self = this;
      Object.keys(this.markers).forEach(function (name) {
        if (!self.layerVisible[name]) return;
        self.markers[name].forEach(function (r) {
          if (r.kind === 'point') pts.push(r);
          else if (r.kind === 'path') paths.push(r);
          else if (r.kind === 'arc') arcs.push(r);
          else if (r.kind === 'ring') rings.push(r);
        });
      });
      this.globe.pointsData(pts).pathsData(paths).arcsData(arcs).ringsData(rings);
    },

    // ── low-level record builders ──
    _pt: function (store, lat, lng, color, sizePx, label, opts) {
      opts = opts || {};
      store.push({
        kind: 'point', lat: lat, lng: lng, color: color,
        radius: Math.max(0.14, sizePx * 0.055), alt: opts.alt != null ? opts.alt : 0.01,
        label: pop(label)
      });
    },
    _path: function (store, coords, color, stroke, label) {
      store.push({ kind: 'path', coords: coords, color: color, stroke: stroke || 0.5, label: pop(label) });
    },
    _arc: function (store, from, to, color, stroke, label) {
      store.push({
        kind: 'arc', sLat: from[0], sLng: from[1], eLat: to[0], eLng: to[1],
        color: color, stroke: stroke || 0.5, label: pop(label)
      });
    },
    _ring: function (store, lat, lng, color, maxR, speed, period) {
      store.push({
        kind: 'ring', lat: lat, lng: lng, ringColor: color,
        maxR: maxR, speed: speed, period: period
      });
    },

    // ═══ LAYERS (data ported 1:1 from the flat map) ═══
    _addConflicts: function () {
      var zones = [
        { lat:48.5, lng:35.5, name:'Ukraine-Russia War', esc:88, detail:'Active frontline — Energy & grain disruption. Oil up, sanctions boost crypto adoption', impact:'HIGH', trend:'up' },
        { lat:31.4, lng:34.4, name:'Gaza Conflict', esc:82, detail:'Red Sea rerouted. Houthi attacks. +15 days transit via Cape', impact:'HIGH', trend:'r' },
        { lat:23.0, lng:57.0, name:'Yemen / Houthi', esc:78, detail:'Red Sea & Gulf of Aden attacks. Insurance 10x. Oil price driver', impact:'HIGH', trend:'up' },
        { lat:15.5, lng:32.5, name:'Sudan Civil War', esc:65, detail:'2nd largest displacement. Gold mining disruption', impact:'MED', trend:'r' },
        { lat:19.8, lng:96.2, name:'Myanmar Military', esc:45, detail:'Civil unrest, jade & gem trade disruption', impact:'LOW', trend:'dn' },
        { lat:23.5, lng:120.5, name:'Taiwan Strait', esc:72, detail:'50% container ships transit. Escalation = supply chain collapse', impact:'HIGH', trend:'up' },
        { lat:38.5, lng:128.0, name:'Korean Peninsula', esc:55, detail:'DPRK missile tests. Regional risk premium', impact:'MED', trend:'r' },
        { lat:33.3, lng:44.3, name:'Iraq Militia', esc:60, detail:'Iran-backed militia attacks. Oil infrastructure risk', impact:'MED', trend:'up' },
        { lat:13.0, lng:122.0, name:'South China Sea', esc:68, detail:'PH-China maritime disputes. Shipping lane risk', impact:'MED', trend:'up' },
        { lat:9.0, lng:38.7, name:'Ethiopia Regional', esc:40, detail:'Tigray aftermath, Amhara insurgency', impact:'LOW', trend:'dn' },
        { lat:36.2, lng:43.1, name:'Syria Instability', esc:52, detail:'Post-conflict zones. Iran-Israel proxy theater', impact:'MED', trend:'r' },
        { lat:4.0, lng:72.0, name:'Indian Ocean Piracy', esc:35, detail:'Somali piracy resurgence near Indian shipping', impact:'LOW', trend:'up' }
      ];
      var self = this;
      zones.forEach(function (z) {
        var escColor = z.esc > 75 ? '#FF3D5A' : z.esc > 50 ? '#f0b429' : '#64748b';
        var trendSym = z.trend === 'up' ? '↑' : z.trend === 'dn' ? '↓' : '→';
        var label = '<span style="color:#FF3D5A;font-weight:700">⚠ ' + z.name + '</span>' +
          '<div style="display:flex;gap:4px;margin:3px 0;align-items:center"><span style="font-size:8px;color:#6b7280">ESCALATION:</span>' +
          '<div style="flex:1;height:3px;background:#1a1a32;border-radius:2px;overflow:hidden"><div style="width:' + z.esc + '%;height:100%;background:' + escColor + ';border-radius:2px"></div></div>' +
          '<span style="font-size:9px;font-weight:700;color:' + escColor + '">' + z.esc + '/100 ' + trendSym + '</span></div>' +
          '<span style="font-size:9px;color:#9aa4b2">' + z.detail + '</span><br>' +
          '<span style="font-size:8px;font-weight:700;color:' + (z.impact === 'HIGH' ? '#FF3D5A' : '#f0b429') + '">MARKET IMPACT: ' + z.impact + '</span>';
        self._pt(self.markers.conflicts, z.lat, z.lng, '#FF3D5A', z.esc > 75 ? 10 : 7, label, { alt: 0.02 });
        // pulsing ring scaled by escalation
        self._ring(self.markers.conflicts, z.lat, z.lng,
          ['rgba(255,61,90,0.55)', 'rgba(255,61,90,0)'], 2 + z.esc / 18, 2.4, 1400 - z.esc * 6);
      });
    },

    _addShipping: function () {
      var routes = [
        { name:'Asia-Europe (Suez)', color:'#00C8FF', weight:2,
        ...
        { name:'Asia-Europe (Cape)', color:'#FF8C00', weight:1.5,
        ...
        { name:'Trans-Pacific', color:'#00E87A', weight:2,
          points:[[22.3,114.2],[25,130],[35,150],[37,170],[35,-170],[35,-155],[34,-140],[34,-118]],
          vol:'$1.2T/yr', detail:'China-US decoupling risk' },
        { name:'Persian Gulf Oil', color:'#FF3D5A', weight:2.5,
          points:[[26.2,56.3],[24,58],[22,62],[18,57],[14,50],[12.6,43.3]],
          vol:'21M bbl/day', detail:'21% global oil via Hormuz' },
        { name:'Malacca Strait', color:'#f0b429', weight:2,
          points:[[1.3,103.8],[2.5,101.5],[4,100],[6,98],[8,96]],
          vol:'25% trade', detail:'60% China oil imports' },
        { name:'Panama Canal', color:'#8b5cf6', weight:2,
          points:[[9,-79.5],[9.3,-79.9],[10,-80],[12,-82],[15,-85],[20,-86],[25,-82],[30,-78]],
          vol:'5% trade', detail:'Drought: 24/day (norm 36-40)' },
        { name:'North Sea / Baltic', color:'#00C8FF', weight:1.5,
          points:[[51.5,1],[54,4],[56,8],[57,12],[59,18],[60,25]],
          vol:'EU energy', detail:'Russian oil sanctions rerouting' },
        { name:'Indian Ocean', color:'#ff6b9d', weight:1.5,
          points:[[19,72.8],[15,68],[10,60],[8,52],[12.6,43.3]],
          vol:'India crude', detail:'85% import dependent. INR/Nifty impact' }
      ];
      var self = this;
      routes.forEach(function (r) {
        self._path(self.markers.shipping, r.points, r.color, r.weight * 0.32,
          '<span style="color:' + r.color + ';font-weight:700">🚢 ' + r.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + r.vol + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + r.detail + '</span>');
      });
    },

    _addChokepoints: function () {
      var cps = [
        { lat:30, lng:32.5, name:'Suez Canal', status:'⚠ DISRUPTED', color:'#FF5C5C', detail:'12% global trade. Houthi attacks. Insurance +1000%' },
        { lat:26.5, lng:56.3, name:'Strait of Hormuz', status:'⚠ ELEVATED', color:'#F7B955', detail:'21% global oil. Iran closure = oil doubles' },
        { lat:1.3, lng:103.8, name:'Malacca Strait', status:'● OPERATIONAL', color:'#39E58C', detail:'25% global trade. China lifeline' },
        { lat:9, lng:-79.5, name:'Panama Canal', status:'⚠ RESTRICTED', color:'#F7B955', detail:'5% trade. Drought: 24/day' },
        { lat:12.6, lng:43.3, name:'Bab el-Mandeb', status:'🔴 HIGH RISK', color:'#FF5C5C', detail:'4.8M bbl/day. Houthi attacks' },
        { lat:-34.5, lng:18.5, name:'Cape of Good Hope', status:'● HEAVY TRAFFIC', color:'#F7B955', detail:'Suez alt. +15 days. 2x traffic' },
        { lat:61, lng:28, name:'Danish Straits', status:'⚠ MONITORING', color:'#F7B955', detail:'Russian shadow fleet transits' },
        { lat:41.2, lng:29, name:'Turkish Straits', status:'● OPERATIONAL', color:'#39E58C', detail:'Black Sea grain/oil' }
      ];
      var self = this;
      cps.forEach(function (cp) {
        self._pt(self.markers.chokepoints, cp.lat, cp.lng, cp.color, 8,
          '<span style="color:' + cp.color + ';font-weight:700">⚓ ' + cp.name + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + cp.detail + '</span><br>' +
          '<span style="font-size:8px;font-weight:700;color:' + cp.color + '">' + cp.status + '</span>', { alt: 0.015 });
      });
    },

    _addSanctions: function () {
      var zones = [
      ...
      var self = this;
      zones.forEach(function (z) {
        var sc = z.severity === 'FULL' ? '#FF5C5C' : z.severity === 'SECTORAL' ? '#8b5cf6' : '#9aa4b2';
        self._pt(self.markers.sanctions, z.lat, z.lng, '#8b5cf6', 6,
          '<span style="color:#8b5cf6;font-weight:700">🚫 ' + z.name + '</span><br>' +
          '<span style="font-size:8px;color:' + sc + ';font-weight:600">SEVERITY: ' + z.severity + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + z.detail + '</span>');
      });
    },
    ...
    _addEnergy: function () {
      ...
      hubs.forEach(function (h) {
        self._pt(self.markers.energy, h.lat, h.lng, '#39E58C', 6,
          '<span style="color:#39E58C;font-weight:700">⚡ ' + h.name + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + h.detail + '</span>');
      });
    },
    ...
    _addWeather: function () {
      ...
      evs.forEach(function (ev) {
        self._pt(self.markers.weather, ev.lat, ev.lng, '#F7B955', 7,
          '<b>' + ev.name + '</b><br><span style="font-size:9px;color:#9aa4b2">' + ev.detail + '</span>');
      });
    },

    _addOilFlows: function () {
      var flows = [
      ...
      flows.forEach(function (f) {
        self._arc(self.markers.oilFlows, f.from, f.to, f.color, 0.55,
          '<span style="color:' + f.color + ';font-weight:700">🛢 ' + f.name + '</span><br><span style="font-size:9px">' + f.vol + '</span>');
      });
    },

    _addMilitary: function () {
      var bases = [
      ...
      bases.forEach(function (b) {
        ...
        self._pt(self.markers.military, b.lat, b.lng, tc, 5,
          '<span style="color:' + tc + ';font-weight:700">★ ' + b.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF;font-weight:600">' + b.type + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + b.detail + '</span>');
      });
    },

    _addTradeCorridors: function () {
      var corridors = [
        { name:'Belt & Road — Maritime Silk Road', color:'#ef4444', weight:2.5,
          points:[[31.2,121.5],[22.3,114.2],[1.3,103.8],[6,80],[10,60],[12.6,43.3],[30,32.5],[37,15],[43.8,7.7]],
          detail:'$1T+ investment. 150+ countries. China global trade network', vol:'40% global trade' },
        { name:'Belt & Road — Land Bridge', color:'#ef4444', weight:2,
          points:[[39.9,116.4],[47,86],[41.3,69.3],[41,44.8],[41.7,44.8],[41,29],[48.1,11.6],[52.5,13.4]],
          detail:'China-Europe rail freight. 15 days vs 45 sea. Kazakhstan corridor', vol:'$750B/yr' },
        { name:'IMEC (India-Middle East-Europe)', color:'#22c55e', weight:2.5,
          points:[[19,72.8],[25,56],[26.5,50],[30,35],[32,34.8],[31.3,32.3],[37,15],[38.7,-9.1]],
          detail:'US/India/EU counter to BRI. Rail + sea + data. Saudi-Israel normalization driver', vol:'$6T trade zone' },
        { name:'CPEC (China-Pakistan)', color:'#f97316', weight:2,
          points:[[39.5,76],[36.2,74],[35.9,72],[33.7,73],[30.4,66.9],[25,66.9]],
          detail:'$62B investment. Gwadar port. Strategic access to Arabian Sea', vol:'$62B corridor' },
        { name:'INSTC (India-Russia via Iran)', color:'#8b5cf6', weight:1.8,
          points:[[19,72.8],[22,60],[28,53],[35.7,51.4],[40,50],[42,47],[55.7,37.6]],
          detail:'North-South Transport Corridor. India-Iran-Russia. Sanctions bypass', vol:'$170B target' },
        { name:'Trans-African Highway', color:'#f0b429', weight:1.5,
          points:[[36.8,10.2],[9,38.7],[0,32.6],[-1.3,36.8],[-6.8,39.3],[-15.4,28.3],[-25.7,28.2],[-33.9,18.4]],
          detail:'Cairo-Cape Town. AfCFTA integration. Resource extraction corridors', vol:'$3.4T AfCFTA' },
        { name:'Arctic Northern Sea Route', color:'#00C8FF', weight:1.8,
          points:[[70,30],[72,50],[73,80],[72,110],[70,140],[68,170],[65,-170],[60,-160]],
          detail:'Russia-controlled. 40% shorter Asia-Europe. Climate-enabled. LNG carriers', vol:'35M tons/yr' }
      ];
      var self = this;
      corridors.forEach(function (c) {
        self._path(self.markers.tradeCorridors, c.points, c.color, c.weight * 0.32,
          '<span style="color:' + c.color + ';font-weight:700">🛤 ' + c.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + c.vol + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + c.detail + '</span>');
      });
    },

    _addLngTerminals: function () {
      var terminals = [
      ...
      terminals.forEach(function (t) {
        var sc = t.status === 'SANCTIONED' || t.status === 'UNDER THREAT' ? '#FF5C5C' : t.status === 'EXPANDING' ? '#39D5FF' : t.status === 'DELAYED' ? '#F7B955' : '#39E58C';
        self._pt(self.markers.lngTerminals, t.lat, t.lng, sc, 5,
          '<span style="color:' + sc + ';font-weight:700">🔥 ' + t.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + t.cap + '</span><br>' +
          '<span style="font-size:8px;font-weight:600;color:' + sc + '">' + t.status + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + t.detail + '</span>');
      });
    },

    _addGrainRoutes: function () {
      var routes = [
        { name:'Black Sea Grain (Ukraine)', color:'#f0b429', weight:2,
        ...
        { name:'US Gulf → Global', color:'#3b82f6', weight:2,
        ...
        { name:'Argentina → Asia', color:'#22c55e', weight:1.8,
        ...
        { name:'Australia → Asia', color:'#8b5cf6', weight:1.8,
        ...
        { name:'Brazil → China', color:'#00E87A', weight:1.8,
        ...
        { name:'India Grain Exports', color:'#f97316', weight:1.5,
      ...
      var self = this;
      routes.forEach(function (r) {
        self._path(self.markers.grainRoutes, r.points, r.color, r.weight * 0.32,
          '<span style="color:' + r.color + ';font-weight:700">🌾 ' + r.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + r.vol + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + r.detail + '</span>');
      });
    ...
    _addSemiconductors: function () {
      var nodes = [
        ...
        { lat:52.7, lng:13.3, name:'Intel Magdeburg (Germany)', detail:'€30B megafab. EU Chips Act. 2027 target', role:'NEW FAB', risk:'LOW' },
      ...
      nodes.forEach(function (n) {
        ...
        self._pt(self.markers.semiconductors, n.lat, n.lng, rc, 6,
          '<span style="color:' + rc + ';font-weight:700">🔬 ' + n.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF;font-weight:600">' + n.role + '</span> · <span style="font-size:8px;color:' + rc + ';font-weight:600">RISK: ' + n.risk + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + n.detail + '</span>');
      });
    },

    _addRareEarths: function () {
      var sites = [
      ...
      sites.forEach(function (s) {
        var rc = s.role === 'MINE' ? '#F7B955' : s.role === 'PROCESSING' ? '#FF8C00' : '#9aa4b2';
        self._pt(self.markers.rareEarths, s.lat, s.lng, rc, 5,
          '<span style="color:' + rc + ';font-weight:700">⛏ ' + s.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF;font-weight:600">' + s.role + '</span> · <span style="font-size:8px;color:#F7B955">' + s.share + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + s.detail + '</span>');
      });
    },

    _addPipelines: function () {
      var pipes = [
        { name:'Nord Stream (Sabotaged)', color:'#ef4444', weight:2,
        ...
        { name:'TurkStream', color:'#f97316', weight:2,
        ...
        { name:'Druzhba (Friendship) Oil', color:'#8b5cf6', weight:1.8,
        ...
        { name:'TAPI Pipeline', color:'#22c55e', weight:1.5,
        ...
        { name:'East-West Pipeline (India)', color:'#3b82f6', weight:1.5,
        ...
        { name:'Trans-Anatolian (TANAP)', color:'#00C8FF', weight:1.8,
        ...
        { name:'Power of Siberia (China)', color:'#ef4444', weight:2,
        ...
        { name:'East Africa Crude Pipeline', color:'#f0b429', weight:1.5,
      ...
      pipes.forEach(function (p) {
        var sc = p.status === 'DESTROYED' ? '#FF5C5C' : p.status === 'STALLED' ? '#F7B955' : p.status === 'OPERATIONAL' ? '#39E58C' : '#39D5FF';
        self._path(self.markers.pipelines, p.points, p.color, p.weight * 0.3,
          '<span style="color:' + p.color + ';font-weight:700">🔵 ' + p.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + p.vol + '</span> · <span style="font-size:8px;color:' + sc + ';font-weight:600">' + p.status + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + p.detail + '</span>');
      });
    },

    _addSubmarineCables: function () {
      var cables = [
        { name:'SEA-ME-WE 6', color:'#00C8FF', weight:1.5,
        ...
        { name:'PEACE Cable (China)', color:'#ef4444', weight:1.5,
        ...
        { name:'JUPITER (Trans-Pacific)', color:'#8b5cf6', weight:1.5,
        ...
        { name:'India-Europe Xpress (IEX)', color:'#f97316', weight:1.5,
        ...
        { name:'Arctic Connect', color:'#00C8FF', weight:1.2,
      ...
      cables.forEach(function (c) {
        self._path(self.markers.submarineCables, c.points, c.color, c.weight * 0.28,
          '<span style="color:' + c.color + ';font-weight:700">🔌 ' + c.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + c.cap + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + c.detail + '</span>');
      });
    },

    _addDisputedZones: function () {
      var zones = [
        { lat:12, lng:114, name:'South China Sea (Nine-Dash Line)', maxR:9, detail:'$5.3T trade transit. China vs PH/VN/MY/TW. Artificial islands. Oil/gas reserves', border:'#ef4444' },
        { lat:30, lng:125, name:'East China Sea (Senkaku/Diaoyu)', maxR:4, detail:'Japan vs China. Oil/gas. ADIZ overlap. US alliance trigger', border:'#f0b429' },
        { lat:80, lng:0, name:'Arctic Claims', maxR:11, detail:'Russia/Canada/Norway/US/Denmark. Oil/gas/minerals. Northern Sea Route', border:'#00C8FF' },
        { lat:32.5, lng:48, name:'Shatt al-Arab (Iran-Iraq)', maxR:2.5, detail:'Waterway dispute. Oil terminal access. Historical trigger', border:'#8b5cf6' },
        { lat:25, lng:68.5, name:'Sir Creek (India-Pakistan)', maxR:2, detail:'Gujarat-Sindh maritime border. Fishing + hydrocarbon rights', border:'#22c55e' },
        { lat:-53, lng:-60, name:'Falklands/Malvinas', maxR:4, detail:'UK vs Argentina. Oil exploration. Southern Atlantic control', border:'#3b82f6' },
        { lat:28, lng:34, name:'Red Sea EEZ Disputes', maxR:3, detail:'Egypt/Saudi/Sudan maritime boundaries. Oil exploration rights', border:'#ff6b9d' }
      ];
      var self = this;
      zones.forEach(function (z) {
        var rgb = self._hexRgb(z.border);
        self._ring(self.markers.disputedZones, z.lat, z.lng,
          ['rgba(' + rgb + ',0.5)', 'rgba(' + rgb + ',0)'], z.maxR, 1.3, 2600);
        self._pt(self.markers.disputedZones, z.lat, z.lng, z.border, 4,
          '<span style="color:' + z.border + ';font-weight:700">⚔ ' + z.name + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + z.detail + '</span>');
      });
    },

    _addFtaZones: function () {
      var zones = [
      ...
      zones.forEach(function (z) {
        var rgb = self._hexRgb(z.border);
        self._ring(self.markers.ftaZones, z.lat, z.lng,
          ['rgba(' + rgb + ',0.4)', 'rgba(' + rgb + ',0)'], 14, 1.0, 3200);
        self._pt(self.markers.ftaZones, z.lat, z.lng, z.border, 5,
          '<span style="color:' + z.border + ';font-weight:700">🤝 ' + z.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF">' + z.gdp + ' GDP</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + z.detail + '</span>');
      });
    },

    _addNuclearFacilities: function () {
      var facilities = [
      ...
      facilities.forEach(function (f) {
        ...
        self._pt(self.markers.nuclearFacilities, f.lat, f.lng, rc, 5,
          '<span style="color:' + rc + ';font-weight:700">☢ ' + f.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF;font-weight:600">' + f.type + '</span> · <span style="font-size:8px;color:' + rc + ';font-weight:600">RISK: ' + f.risk + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + f.detail + '</span>');
      });
    },

    _addNavalFleets: function () {
      var fleets = [
      ...
      fleets.forEach(function (f) {
        ...
        self._pt(self.markers.navalFleets, f.lat, f.lng, tc, 6,
          '<span style="color:' + tc + ';font-weight:700">⚓ ' + f.name + '</span><br>' +
          '<span style="font-size:8px;color:#39D5FF;font-weight:600">' + f.type + '</span><br>' +
          '<span style="font-size:9px;color:#9aa4b2">' + f.detail + '</span>');
      });
    },
    ...
    loadQuakes: function () {
      ...
      var fetchFn = function () { return fetch(url).then(function (r) { return r.json(); }); };
      var p = window.NxCache ? NxCache.fetch('quakes', 300000, fetchFn) : fetchFn();
      Promise.resolve(p).then(function (d) {
        self.markers.quakes = [];
        (d && d.features || []).forEach(function (f) {
          var c = f.geometry.coordinates, lng = c[0], lat = c[1], mag = f.properties.mag;
          self._pt(self.markers.quakes, lat, lng, '#ff8c00', Math.max(4, mag * 2),
            '<b>M' + mag + ' Earthquake</b><br><span style="font-size:9px;color:#9aa4b2">' + f.properties.place + '</span>');
        });
        self._render();
